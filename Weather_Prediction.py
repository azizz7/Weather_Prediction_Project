...
def data_get():
    city = city_name.get()
    data = requests.get(
        f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid=1b3537245f6d8cf3f1d87bdae9149a9a"
    ).json()
...
com = ttk.Combobox(
    win,
    text="Weather Predicted By The GOAT",
    values=list_name,
...
done_button = Button(
    win, text="Done", font=("Times New Roman", 20, "bold"), command=data_get
)
